...
function getPolynom(...args) {
  return function l(x) {
    const k = args.slice().reverse().reduce((ac, el, idx) => ac + el * (x ** idx), 0);
    return k;
  };
}
...
function logger(func, logFunc) {
  return (...x) => {
    const k = JSON.stringify(x).slice(1, -1);
    logFunc(`${func.name}(${k}) starts`);
    const res = func(...x);
    logFunc(`${func.name}(${k}) ends`);
    return res;
  };
}
